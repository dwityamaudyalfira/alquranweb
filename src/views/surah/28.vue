...
export default {
  ...
  methods: {
    async getChapter(){
      fetch('https://api.quran.com/api/v4/chapters/28?language=id',{
        method: 'GET'
      })
        .then(response => {
          if(response.ok){
    ...
    async getVerses(){
      fetch('https://api.quran.com/api/v4/quran/verses/uthmani?chapter_number=28', {
        method: 'GET'
      })
        .then(res =>{
          if(res.ok){
    ...
    async getInfo(){
      fetch('https://api.quran.com/api/v4/chapters/28/info?language=id', {
        method: 'GET'
      })
        .then(res =>{
          if(res.ok){
